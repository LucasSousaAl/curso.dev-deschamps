...
function tagStyle(t) {
  const s = TAG_MAP[t] || TAG_DEF;
  return { background: s[0], color: s[1], border: `1px solid ${s[2]}` };
}
...
function launchConfetti() {
  ...
  for (let i = 0; i < 75; i++)
    setTimeout(() => {
      ...
      el.className = "nj-confetti-p";
      const sz = 5 + Math.random() * 9;
      el.style.cssText = `left:${5 + Math.random() * 90}vw;top:-14px;background:${C[Math.floor(Math.random() * C.length)]};width:${sz}px;height:${sz}px;border-radius:${Math.random() > 0.5 ? "50%" : "2px"};animation-duration:${1.4 + Math.random() * 1.6}s;`;
...
function VoteOverlay({ gameId, count, onVote }) {
  ...
  return (
    ...
    >
      <div
        className={pop ? "nj-vnum-pop" : ""}
        style={{
          fontFamily: "'Bebas Neue',sans-serif",
          fontSize: "3.8rem",
          lineHeight: 1,
          color: count > 0 ? "#e74c3c" : "#fff",
      ...
      <div style={{ display: "flex", gap: 10 }}>
        {[
          { d: 1, l: "+", bg: "#c0392b", fg: "#fff", border: "none" },
          {
            d: -1,
            l: "−",
...
function GameCard({
  ...
  const bord =
    isLeader && votingMode
      ? "#c0392b"
      : isSelected && !votingMode
        ? "#f0a500"
        : "rgba(192,57,43,.28)";
  const shad =
    isLeader && votingMode
      ? "0 0 0 2px #c0392b,0 0 45px rgba(192,57,43,.5)"
      : isSelected && !votingMode
        ? "0 0 0 2px #f0a500,0 0 40px rgba(243,156,18,.3)"
        : hov && !votingMode
          ? "0 18px 60px rgba(0,0,0,.7),0 0 28px rgba(192,57,43,.55)"
          : "none";
  ...
  return (
    <article
      ...
      style={{
        ...
        borderRadius: 4,
        overflow: "hidden",
        cursor: votingMode || isEliminated ? "default" : "pointer",
        position: "relative",
        border: `1px solid ${bord}`,
        boxShadow: shad,
        transition:
          "transform .35s cubic-bezier(.22,1,.36,1),box-shadow .3s,opacity .6s,border-color .25s",
        transform:
          hov && !votingMode && !isEliminated
            ? "translateY(-6px) scale(1.014)"
            : "none",
    ...
    >
      ...
      {!votingMode && !isEliminated && (
        <button
          ...
          onMouseEnter={(e) => {
            e.currentTarget.style.background = "rgba(192,57,43,.75)";
      ...
      >
        ...
        <div
          style={{
            ...
            inset: 0,
            background:
              "linear-gradient(to top,rgba(8,8,8,1) 0%,rgba(8,8,8,.06) 55%,transparent 100%)",
          }}
        />
        <div
          style={{
        ...
        {!votingMode && isSelected && (
          <div
            style={{
              ...
              fontSize: ".8rem",
              zIndex: 15,
              animation: "nj-pop .3s cubic-bezier(.34,1.56,.64,1) both",
            }}
          >
            ✓
          </div>
        ...
        {votingMode && !isEliminated && (
          <VoteOverlay gameId={game.id} count={voteCount} onVote={onVote} />
        )}
      </div>
...
function SearchBar({ onAdd, existingIds }) {
  ...
  useEffect(() => {
    ...
    timer.current = setTimeout(async () => {
      ...
      setOpen(true);
      try {
        const r = await searchSteamGames(q.trim());
        setRes(r);
      } catch (e) {
      ...
      {open && (
        <div className="nj-dropdown">
          ...
          {loading && (
            ...
            >
              <div className="nj-spinner" style={{ width: 16, height: 16 }} />{" "}
              Buscando na Steam...
            </div>
          )}
          {res.map((g) => {
  ...
  return (
    <div
      ...
      style={{
        ...
        animation: "nj-fdDown .3s ease both",
      }}
    >
      <div
        ...
        style={{
          ...
          animation: "nj-fdUp .4s cubic-bezier(.34,1.56,.64,1) both",
        }}
      >
        {children}
      </div>
        ...
        <>
          ...
          <div
            style={{
              ...
              alignItems: "center",
              justifyContent: "center",
              textShadow: "0 0 30px rgba(240,165,0,.7)",
            }}
          >
            {reel || "..."}
          </div>
        ...
        <>
          ...
          <img
            ...
            style={{
              ...
              boxShadow: "0 0 40px rgba(243,156,18,.2)",
            }}
          />
          <p
            style={{
          ...
          >
            <Btn col="gold" onClick={spin}>
              🎲 De Novo
            </Btn>
            <Btn col="ghost" onClick={close}>
              Fechar
            </Btn>
          </div>
        </>
...
export default function App() {
  useEffect(() => {
    ...
    s.id = "nj-css";
    if (!document.getElementById("nj-css")) {
  ...
  const leaderId =
    maxV > 0 ? activeGames.find((g) => (votes[g.id] || 0) === maxV)?.id : null;
  const existingIds = new Set(games.map((g) => Number(g.appid || g.id)));

  function addGame(game) {
    setGames((p) => {
      if (p.find((x) => x.id === game.id)) return p;
      return [...p, game];
    });
  }
  ...
  function startVoting() {
    if (activeGames.length < 2) {
      alert("Adicione pelo menos 2 jogos para votar!");
      return;
    }
    setVotes({});
  ...
  function handleVote(id, d) {
    setVotes((p) => ({ ...p, [id]: Math.max(0, (p[id] || 0) + d) }));
  }

  function tryEliminate() {
    if (activeGames.length <= 1) return;
    if (maxV === 0) {
      alert("Ninguém votou ainda! Use os botões + para votar.");
      return;
    }
  ...
  function confirmElim() {
    const game = modals.elim;
    if (!game) return;
    setModals((m) => ({ ...m, elim: null }));
    setAnimId(game.id);
    setTimeout(() => {
      setEliminated((p) => {
        const n = new Set(p);
      ...
      const remaining = games.filter(
        (g) => !eliminated.has(g.id) && g.id !== game.id,
      );
      if (remaining.length === 1) {
        setTimeout(() => {
          setVoting(false);
          setModals((m) => ({ ...m, winner: remaining[0] }));
          launchConfetti();
        }, 700);
      }
    }, 460);
  ...
  return (
    <div id="njroot">
      ...
      >
        <div
          style={{
            ...
            inset: 0,
            background:
              "radial-gradient(ellipse at 50% 100%,rgba(192,57,43,.18) 0%,transparent 70%)",
            pointerEvents: "none",
          }}
        />
        <p
          style={{
            ...
            color: "#c0392b",
            marginBottom: 16,
            animation: "nj-fdDown .8s ease both",
          }}
        >
          ⚔ Votação da Noite ⚔
        </p>
        <h1
          style={{
            ...
            lineHeight: 1.06,
            background:
              "linear-gradient(135deg,#fff 20%,#f0a500 55%,#e74c3c 90%)",
            WebkitBackgroundClip: "text",
            WebkitTextFillColor: "transparent",
            backgroundClip: "text",
            filter: "drop-shadow(0 0 40px rgba(192,57,43,.5))",
            animation: "nj-fdDown .9s .1s ease both",
          }}
        >
          Noite de Jogos
        </h1>
        <p
          style={{
            ...
            color: "#555",
            letterSpacing: ".08em",
            animation: "nj-fdDown .9s .2s ease both",
          }}
        >
        ...
        <div
          style={{
            ...
            animation: "nj-fdDown .9s .3s ease both",
          }}
        />
      </section>
      ...
      {!voting && (
        <div
          style={{ maxWidth: 600, margin: "0 auto", padding: "0 24px 36px" }}
        >
      ...
      {voting && (
        ...
        >
          <div>
            ...
            >
              ☠ RODADA <span style={{ color: "#c0392b" }}>{round}</span> —
              ELIMINAÇÃO
            </div>
            <div
              style={{
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <Btn
              col="ghost"
              onClick={() => setVotes({})}
              ex={{ fontSize: "1rem", padding: "8px 14px" }}
            >
              ↺ Zerar
            </Btn>
            <Btn
              col="primary"
              onClick={tryEliminate}
              ex={{ fontSize: "1rem", padding: "8px 20px" }}
            >
              💀 Eliminar Mais Votado
            </Btn>
            <Btn
              col="ghost"
              onClick={stopVoting}
      ...
      {games.length > 0 && !voting && (
        <section style={{ textAlign: "center", padding: "0 20px 70px" }}>
          ...
          >
            ...
            >
              {selectedGames.length === 0 ? (
                ...
                >
                  Nenhum selecionado — clique nas cartas!
                </span>
              ) : (
                selectedGames.map((g) => (
                  <span
          ...
          >
            <Btn
              col="primary"
              ex={{ fontSize: "1.3rem", padding: "13px 40px" }}
              onClick={() =>
            ...
            <Btn
              ...
              onClick={() =>
                games.length
                  ? setModals((m) => ({ ...m, sorteio: true }))
                  : alert("Adicione jogos primeiro!")
              }
            >
              🎲 Sortear Jogo
            </Btn>
      ...
      <Modal
        show={modals.confirm}
        onClose={() => setModals((m) => ({ ...m, confirm: false }))}
      >
        ...
        <Btn
          col="ghost"
          onClick={() => setModals((m) => ({ ...m, confirm: false }))}
        >
          Voltar
        </Btn>
      </Modal>
      ...
      <Modal
        show={!!modals.elim}
        onClose={() => setModals((m) => ({ ...m, elim: null }))}
      >
        {modals.elim && (
          <>
            ...
            >
              {votes[modals.elim.id] || 0}{" "}
              {(votes[modals.elim.id] || 0) === 1 ? "VOTO" : "VOTOS"}
            </div>
            <img
              src={modals.elim.header_image}
              alt={modals.elim.name}
              style={{
                width: "100%",
                borderRadius: 3,
                marginBottom: 22,
            ...
            >
              <Btn col="primary" onClick={confirmElim}>
                ✔ Confirmar Eliminação
              </Btn>
              <Btn
                col="ghost"
                onClick={() => setModals((m) => ({ ...m, elim: null }))}
              >
                Cancelar
              </Btn>
            </div>
      ...
      <Modal
        show={!!modals.winner}
        onClose={() => setModals((m) => ({ ...m, winner: null }))}
        bord="rgba(243,156,18,.5)"
        glow="rgba(243,156,18,.35)"
      >
        {modals.winner && (
          <>
            <div
              style={{
                ...
                marginBottom: 10,
                display: "inline-block",
                animation: "nj-bounce .6s ease infinite alternate",
              }}
            >
              🏆
            </div>
            ...
            <img
              src={modals.winner.header_image}
              alt={modals.winner.name}
              style={{
                width: "100%",
                borderRadius: 3,
                marginBottom: 18,
            ...
            >
              {modals.winner.short_description}
            </p>
            <Btn
              col="primary"
              ex={{ fontSize: "1.3rem", padding: "13px 40px" }}
              onClick={() => setModals((m) => ({ ...m, winner: null }))}
            >
              🎮 Vamos Jogar!
            </Btn>
          </>
      ...
      <SorteioModal
        show={modals.sorteio}
        games={activeGames}
        onClose={() => setModals((m) => ({ ...m, sorteio: false }))}
      />
    </div>
  );
}
